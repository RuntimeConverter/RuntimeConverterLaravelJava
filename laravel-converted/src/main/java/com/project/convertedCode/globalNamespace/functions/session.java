...
public class session extends FunctionBaseRegular {
    ...
    public Object call(RuntimeEnv env, Object... args) {
        ...
        if (function_is_null.f.env(env).call(key).getBool()) {
            return ZVal.assign(app.f.env(env).call("session").value());
        }

        if (function_is_array.f.env(env).call(key).getBool()) {
            return ZVal.assign(
                    env.callMethod(
                            app.f.env(env).call("session").value(), "put", session.class, key));
        }

        return ZVal.assign(
                env.callMethod(
                        app.f.env(env).call("session").value(),
                        "get",
                        session.class,
                        key,
                        _pDefault));
