...
public class ValidateRequestListener extends RuntimeClassBase implements EventSubscriberInterface {

    public ValidateRequestListener(RuntimeEnv env, Object... args) {
        super(env);
    }

    public ValidateRequestListener(NoConstructor n) {
        super(n);
    }
    ...
    public Object onKernelRequest(RuntimeEnv env, Object... args) {
        ...
        Object request = null;
        if (!ZVal.isTrue(env.callMethod(event, "isMasterRequest", ValidateRequestListener.class))) {
            return null;
        }

        request = env.callMethod(event, "getRequest", ValidateRequestListener.class);
        if (ZVal.isTrue(
                StaticMethodUtils.getStaticBaseClass(request, env)
                        .callUnknownStaticMethod(env, "getTrustedProxies"))) {
            env.callMethod(request, "getClientIps", ValidateRequestListener.class);
        }

        env.callMethod(request, "getHost", ValidateRequestListener.class);
        return null;
    }
                    ...
                    ZVal.newArray(
                            new ZPair(
                                    ...
                                    ZVal.newArray(
                                            new ZPair(
                                                    0,
                                                    ZVal.arrayFromList("onKernelRequest", 256))))));
        }
    }
