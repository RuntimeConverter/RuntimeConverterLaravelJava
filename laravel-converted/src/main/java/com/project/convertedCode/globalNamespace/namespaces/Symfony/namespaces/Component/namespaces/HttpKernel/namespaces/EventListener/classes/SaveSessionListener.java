...
public class SaveSessionListener extends RuntimeClassBase implements EventSubscriberInterface {

    public SaveSessionListener(RuntimeEnv env, Object... args) {
        super(env);
    }

    public SaveSessionListener(NoConstructor n) {
        super(n);
    }
    ...
    public Object onKernelResponse(RuntimeEnv env, Object... args) {
        ...
        Object session = null;
        if (!ZVal.isTrue(env.callMethod(event, "isMasterRequest", SaveSessionListener.class))) {
            return null;
        }

        session =
                env.callMethod(
                        env.callMethod(event, "getRequest", SaveSessionListener.class),
                        "getSession",
                        SaveSessionListener.class);
        if (ZVal.toBool(session)
                && ZVal.toBool(env.callMethod(session, "isStarted", SaveSessionListener.class))) {
            env.callMethod(session, "save", SaveSessionListener.class);
        }
    ...
    private static final ReflectionClassData runtimeConverterReflectionData =
            ReflectionClassData.builder()
                    .setName("Symfony\\Component\\HttpKernel\\EventListener\\SaveSessionListener")
                    .setLookup(
                            SaveSessionListener.class,
                            MethodHandles.lookup(),
                            RuntimeStaticCompanion.staticCompanionLookup)
                    .setLocalProperties()
                    .setFilename("vendor/symfony/http-kernel/EventListener/SaveSessionListener.php")
